...
class Bot(object):
    ...
    def get_messages(self, slack_args, messages, filter_func):
        history = self.client.api_call("channels.history", **slack_args)
        last_ts = history['messages'][-1]['ts'] if (history and history['has_more']) else False
    ...
    def scrape_slack(self, slack_args, filter_func = lambda x: x):
        results = self.get_messages(slack_args, [], filter_func)

        while results['last_ts']:
            slack_args['latest'] = results['last_ts']
            results = self.get_messages(sc, slack_args, results['messages'], filter_func)
